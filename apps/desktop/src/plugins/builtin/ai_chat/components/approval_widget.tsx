    ...
    return (
      <div class="rounded-xl border border-border bg-bg-secondary p-3 text-xs">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              ...
              <p class="mt-1 truncate text-[11px] text-text-muted">
        ...
        <Show when={props.item.expanded}>
          <pre class="mt-3 max-h-32 overflow-auto rounded-lg bg-bg-primary/70 p-2 text-[11px] wrap-break-word whitespace-pre-wrap text-text-muted">
  ...
  return (
    <div class="rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-xs">
      <div class="flex items-start justify-between gap-3">
        <div>
          <div class="font-medium text-text-primary">{props.item.toolName}</div>
          <div class="mt-1 text-[11px] text-text-muted">{props.item.previewText}</div>
        </div>
        <div class="rounded-full border border-amber-500/40 px-2 py-0.5 text-[11px] text-amber-300">
          {statusLabel()}
        </div>
      </div>

      <pre class="mt-2 max-h-32 overflow-auto rounded-lg bg-bg-primary/70 p-2 text-[11px] wrap-break-word whitespace-pre-wrap text-text-muted">
        {JSON.stringify(props.item.mutation, null, 2)}
      </pre>
      ...
      <div class="mt-3 flex items-center gap-2">
        <Show when={canOpenApprovalDiff(props.item.mutation, props.item.toolName)}>
          <button
            type="button"
            class="rounded-md border border-border bg-bg-secondary px-3 py-1.5 text-[11px] text-text-secondary transition-colors hover:bg-bg-tertiary hover:text-text-primary"
        ...
        <button
          type="button"
          class="rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-1.5 text-[11px] text-emerald-300 transition-colors hover:bg-emerald-500/20"
