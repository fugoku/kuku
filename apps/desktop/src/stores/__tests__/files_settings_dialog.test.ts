...
function installBrowserGlobals(): void {
  ...
  Object.defineProperty(globalThis, "crypto", {
    configurable: true,
    value: {
      randomUUID: vi.fn().mockReturnValueOnce("tab-1").mockReturnValue("tab-next"),
    },
  });
}
...
describe("files settings dialog state", () => {
  ...
  it("can preserve the settings dialog during desktop reset", async () => {
    const files = await loadFilesModule();

    files.openTab("Note", "Note.md", "editor");
    files.resetFilesState({
      preserveSettingsDialog: true,
      settingsTarget: { kind: "category", categoryId: "general" },
    });

    expect(files.filesState.tabs).toHaveLength(0);
    expect(files.filesState.activeTabId).toBeNull();
    expect(files.filesState.settingsDialogOpen).toBe(true);
    expect(files.filesState.settingsTarget).toEqual({
      kind: "category",
      categoryId: "general",
