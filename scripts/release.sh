...
PROD_API_URL="${KUKU_PROD_API_URL:-https://api.kuku.mom}"
PROD_WEB_URL="${KUKU_PROD_WEB_URL:-https://kuku.mom}"
...
write_release_manifest() {
    ...
    local tar_name="Kuku.app.tar.gz"
    local tar_url="https://github.com/${GH_REPO}/releases/download/${version}/${tar_name}"
...
EOF
    ...
    log_info "   Will be served at ${PROD_WEB_URL}/release.json"
}
...
main() {
    ...
    # Web dist must exist before we write the manifest into it.
    build_web
